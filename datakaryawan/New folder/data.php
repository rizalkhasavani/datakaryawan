	...
	<div class="container">
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'delete'){ // mengkonfirmasi jika 'aksi' bernilai 'delete' merujuk pada baris 90 dibawah
				$nama = $_GET['nama']; // ambil nilai nik
				$cek = mysqli_query($koneksi, "SELECT * FROM karyawan WHERE nama='$nama'"); // query untuk memilih entri dengan nik yang dipilih
				if(mysqli_num_rows($cek) == 0){ // mengecek jika tidak ada entri nik yang dipilih
					...
					$delete = mysqli_query($koneksi, "DELETE FROM karyawan WHERE nama='$nama'"); // query untuk menghapus
			...
			<div class="table-responsive">
				<table class="table table-striped table-hover">
					...
					<?php
					if($filter){
						$sql = mysqli_query($koneksi, "SELECT * FROM karyawan WHERE status='$filter' ORDER BY nama ASC"); // query jika filter dipilih
					}else{
						$sql = mysqli_query($koneksi, "SELECT * FROM karyawan ORDER BY nama ASC"); // jika tidak ada filter maka tampilkan semua entri
					}
						...
						while($row = mysqli_fetch_assoc($sql)){ // fetch query yang sesuai ke dalam array
							...
							echo '
								...
								<td>
									...
									<a href="index.php?aksi=delete&nama='.$row['nama'].'" title="Hapus Data" data-toggle="tooltip" onclick="return confirm(\'Anda yakin akan menghapus data '.$row['nama'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
								</td>
							</tr>
							';
